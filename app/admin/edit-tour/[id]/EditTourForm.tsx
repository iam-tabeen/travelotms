...
export default function EditTourForm({ tour }: { tour: any }) {
  ...
  // --- THE FIX: Add State for Departure Type to toggle the Date Picker ---
    ...
    <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-8">
      ...
      <form action={updateTourWithId} className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-6 bg-gray-50 border border-gray-100 rounded-2xl">
          ...
          {/* --- THE FIX: ADDED DEPARTURE SCHEDULE BLOCK --- */}
          <div className="md:col-span-2">
            <label className="block text-xs font-black text-axius-secondary uppercase mb-2" style={{fontFamily: 'var(--font-poppins)', fontWeight:"700" , fontSize:"14px"}}>Departure Schedule</label>
            <select 
              name="departureType" 
              value={departureType}
              onChange={(e) => setDepartureType(e.target.value)}
              className="w-full border border-gray-300 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-axius-primary bg-white font-bold"
            >
            ...
            {departureType === 'CUSTOM_DATE' && (
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6 mt-4 p-4 bg-blue-50/50 rounded-xl border border-blue-100">
                <input 
                  type="date" 
                  name="departureDate" 
                  defaultValue={formattedDate}
                  required
                  className="p-3 bg-white border border-gray-200 rounded-lg outline-none focus:border-[#003580] font-medium text-gray-700 w-full sm:w-auto"
                />
