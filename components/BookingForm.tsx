...
import { submitPublicBooking, validatePublicPromo } from '@/app/actions/public-site';
...
export default function BookingForm({
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const result = await submitPublicBooking({
        tourId,
        fullName: String(formData.get('customerName') || ''),
  ...
  const handleApplyPromo = async () => {
    ...
    try {
      const result = await validatePublicPromo(promoInput);
