...
Sold out              </span>
      ...
      <div className="p-6 flex flex-col flex-grow">
        <div className="text-axius-primary flex gap-2 text-xs font-bold uppercase tracking-widest mb-2">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512" width={"10px"} ><path fill="var(--theme-card)" d="M192 284.4C256.1 269.9 304 212.5 304 144 304 64.5 239.5 0 160 0S16 64.5 16 144c0 68.5 47.9 125.9 112 140.4L128 480c0 17.7 14.3 32 32 32s32-14.3 32-32l0-195.6zM168 96c-30.9 0-56 25.1-56 56 0 13.3-10.7 24-24 24s-24-10.7-24-24c0-57.4 46.6-104 104-104 13.3 0 24 10.7 24 24s-10.7 24-24 24z"/></svg> {tour.destination}
        </div>

        <h2 className="text-2xl font-black mb-4 text-axius-secondary line-clamp-2">
          {tour.title}
        </h2>

        {/* Transport & Accommodation */}
        <div className="space-y-3 mb-6 flex gap-3">
          <div style={{ background: "rgba(0, 0, 0, 0)", border:"2px solid var(--theme-card)", margin: "0px", paddingLeft: "18px", paddingRight:"18px", paddingTop:"8px", paddingBottom:"8px", borderRadius: "50px" }} className="flex items-center  text-gray-600 text-sm font-medium">
  ...
  {getDepartureDisplay(tour)}
        ...
        <div className="flex justify-between items-center mt-auto pt-5 border-t border-gray-100">
          <span  className="text-xl font-black tracking-tight">
            Rs. {tour.basePrice.toLocaleString()} <span style={{color:"gray", fontSize:"14px", fontWeight:"400", fontFamily:"DM Sans, sans-serif" }}>/per person</span>
          </span>
          <Link
            href={`/tours/${tour.id}`}
            style={{background:"var(--theme-button)", whiteSpace:"nowrap"}}
            className="px-5 py-2.5 text-white text-xs font-bold uppercase tracking-widest rounded-lg shadow-md hover:bg-axius-secondary transition duration-300 ease-in-out"
          >
            View Details
          </Link>
        </div>
